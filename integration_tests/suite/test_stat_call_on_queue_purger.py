...
from wazo_purge_db.table_purger import StatCallOnQueuePurger
...
class TestStatCallOnQueuePurger(DAOTestCase):
    ...
    def test_that_StatCallOnQueuePurger_keep_everything_when_no_old_entry(self):
        ...
        id_entry0 = self.add_stat_call_on_queue(
            time=current_time - timedelta(days=days_to_keep - 1)
        )
        id_entry1 = self.add_stat_call_on_queue(
            time=current_time - timedelta(days=days_to_keep - 2)
        )
        id_entry2 = self.add_stat_call_on_queue(
            time=current_time - timedelta(days=days_to_keep - 3)
        )
